...
def statusOnFile(uid):
    ...
    #return r.text
    try:
        with urllib.request.urlopen(server + '/index.php?getinfo=1&stage=' + stage + '&uid=' + str(uid)) as response:
            ...
            if rec[0] == "Stage 1 Testing (VT)":
                ...
                print("Malware Percent: " + rec[1])
                print("Stealth / Clean Percent: " + rec[3])
                print("Sus: " + rec[2])
                stage = stage + 1
...
print("Uploading to server: " + sys.argv[1])
...
uid = s
try:
    thestring = server + "/index.php?uid=" + uid
