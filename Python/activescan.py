...
major = str(major)
minor = str(minor)
print("Windows NT Version " + major + "." + minor + "")
...
if major == "6":
    commondirs = [userpf+"\\Downloads",userpf+"\\Appdata",userpf+"\\Documents",userpf+"\\Desktop",userpf+"\\Music"]
elif major == "5" and minor == "0":
    # Windows 2000
    print("Windows 2000")
    commondirs = [userpf+"\\My Documents",userpf+"\\Start Menu",userpf+"\\Application Data",userpf+"\\Desktop",userpf+"\\Favorites"]
else:
    commondirs = [userpf+"\\Downloads",userpf+"\\Appdata",userpf+"\\Documents",userpf+"\\Desktop",userpf+"\\Music"]
debug = 0
canscantimely = 0
...
def requestscan(directory):
    print("requestscan(): "+directory)
    print("Checking exclusions:")
    notexcluded = 1
    for i in range(len(exclusions)):
        print("Checking exclusions: " + i + " " + exclusions[i])
        print(directory)
        if str(exclusions[i]) in directory:
            print("its in there!")
            notexcluded = 0
    ...
    if notexcluded == 1:  
        ...
        subprocess.call('sscan.bat "' + directory + '" activescan', creationflags=CREATE_NO_WINDOW)
...
def checkfiles():
    ...
    for i in range(len(commondirs)):
        ...
        try:
            ...
            print("That director at "+commondirs[i]+" Doesnt seen to exist!")
            commondirs[i] == ""+userpf+""
            print(commondirs)
            print(commondirs[i])
            lst = os.listdir(userpf) # your directory path
        ...
        if ranOnce == 0:
            print("----- in append! i: "+str(i)+" -----")
            filesAndFolders1.append(''+commondirs[i]+' : '+str(nummain)+'')
            #print(filesAndFolders1)
            
            print("Number of files in "+commondirs[i]+": "+str(cpt)+" using cpt\n")
            filesAndFolders2.append(''+commondirs[i]+' : '+str(cpt)+'')
        ...
        if ranOnce == 1:
            ...
            folder1String = ""+commondirs[i]+" : "+str(nummain)+""
            if folder1String != filesAndFolders1[i]:
                print("---------- A: Change detected at commondirs: "+str(i)+" ----------")
                scanreq1 = 1
            ...
            folder2String = ""+commondirs[i]+" : "+str(cpt)+""
            if folder2String != filesAndFolders2[i]:
                print("---------- B: Change detected at commondirs: "+str(i)+" ----------")
                scanreq2 = 1
                time.sleep(5) # for debug 
            ...
            if scanreq1 == 1 or scanreq2 == 1:
                print("Will run a scan at: "+commondirs[i]+"")
                # scan that dir:
...
def mainloop():
    ...
    subprocess.call("sscan.bat " + commondirs[0] + " activescan", creationflags=CREATE_NO_WINDOW)
    print("scan finish, delaying")
    ...
    if os.path.isfile("threats.txt") == True: # threat o noee!!
        mainloop() # restart and wait till threats are all gone okie?
    subprocess.call("sscan.bat " + commondirs[1] + " activescan", creationflags=CREATE_NO_WINDOW)
    print("scan finish, delaying")
    time.sleep(scantime + 14)
    print("checking if threat")
    print(os.path.isfile("threats.txt"))
    ...
    if os.path.isfile("threats.txt") == True: # threat o noee!!
        mainloop() # restart and wait till threats are all gone okie?
    subprocess.call("sscan.bat " + commondirs[2] + " activescan", creationflags=CREATE_NO_WINDOW)
    print("scan finish, delaying")
    time.sleep(scantime + 14)
    print("checking if threat")
    print(os.path.isfile("threats.txt"))
    if os.path.isfile("threats.txt") == True: # threat o noee!!
        mainloop() # restart and wait till threats are all gone okie?
    subprocess.call("sscan.bat " + commondirs[3] + " activescan", creationflags=CREATE_NO_WINDOW)
    mainloop()
                    ...
                    while end == 0:
                        ...
                        if settings[i + ae] == "endexclude" or settings[i + ae] == "endexclude\n":
                            ...
                            if settings[i + ae] == "EXCLUDE" or settings[i + ae] == "EXCLUDE\n":
                                ...
                                exclusions.append(""+settings[i + ae]+"")
                            print(exclusions)
                            ae = ae + 1
            except:
